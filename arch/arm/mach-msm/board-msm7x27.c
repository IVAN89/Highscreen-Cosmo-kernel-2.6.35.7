...
#include <linux/usb/usb_pid.h>
#endif

//#ifdef CONFIG_CCI_LED_AN32155A
//#include <linux/i2c/an32155a.h>
//#endif

#ifdef CONFIG_CCI_HWID
...
#define MSM_PMEM_ADSP_SIZE	0x1300000 //B71000
...
#define BT_RST_N		16
#define BT_REG_ON		91 //BT_PWR_EN
#define K_GPIO_WIFI_IRQ		17
...
//#define CONFIG_CCI_POWER_STATUS
/* ---- WIFI ---- */
...
static int k_wifi_cd = 0;	/* WiFi virtual 'card detect' status */
...
static void *k_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init k_init_wifi_mem(void)
{
	int i;
...
static int
k_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static unsigned int k_wifi_status(struct device *dev)
{
	return k_wifi_cd;
}
int k_wifi_set_carddetect(int val)
{
...
int k_wifi_power(int on)
{
	printk(KERN_INFO "%s: %d\n", __func__, on);

	if (on) {
		msm_sdcc_setup_gpio(2, on); //for wifi setting
	} else {
		msm_sdcc_setup_gpio(2, on); //for wifi setting
	}
	mdelay(100);
...
int k_wifi_reset(int on)
{
	printk(KERN_INFO "%s: do nothing\n", __func__);
	return 0;
}

static struct resource k_wifi_resources[] = {
	[0] = {
		.name		= "wlan_bcm4329_irq",
		.start		= MSM_GPIO_TO_INT(K_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(K_GPIO_WIFI_IRQ),
		.flags          = IORESOURCE_IRQ | IORESOURCE_IRQ_LOWEDGE,
	},
};

static struct wifi_platform_data k_wifi_control = {
	.set_power      = k_wifi_power,
	.set_reset      = k_wifi_reset,
	.set_carddetect = k_wifi_set_carddetect,
	.mem_prealloc	= k_wifi_mem_prealloc,
};

static struct platform_device k_wifi_device = {
        .name           = "wlan_bcm4329",
        .id             = 1,
        .num_resources  = ARRAY_SIZE(k_wifi_resources),
        .resource       = k_wifi_resources,
        .dev            = {
                .platform_data = &k_wifi_control,
        },
};

static int __init k_wifi_init(void)
{
	int ret;

	printk("%s: start\n", __func__);
	k_init_wifi_mem();
	ret = platform_device_register(&k_wifi_device);
        return ret;
}
...
static void brcm4329_init(void)
{
    ...
    printk("[BCM4329] BT_REG_ON=%d, BT_RST_N=%d, WLAN_RESET=%d\n",
...
static struct android_usb_product usb_products[] = {
	{
		.product_id	= CAP_PID_ADB_DISABLE,
		.num_functions	= ARRAY_SIZE(usb_functions_default),
		.functions	= usb_functions_default,
	},
	{
		.product_id	= CAP_PID_ADB_ENABLE,
		.num_functions	= ARRAY_SIZE(usb_functions_default_adb),
		.functions	= usb_functions_default_adb,
	},
	{
		.product_id	= CAP_PID_RNDIS_MODE,//reaff
...
static struct usb_ether_platform_data rndis_pdata = {
	...
	.vendorID	= CAP_VID,
	.vendorDescr	= CAP_MANUFACTURER_NAME,
};
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.product_id	= CAP_PID_ADB_ENABLE,
	.version	= 0x0100,
	.product_name		= CAP_PRODUCT_NAME,
	.manufacturer_name	= CAP_MANUFACTURER_NAME,
...
static void hw_board_version(void)
{
	cci_smem_value_t *smem_cci_smem_value;

	smem_cci_smem_value = smem_alloc( SMEM_CCI_SMEM_VALUE, sizeof( cci_smem_value_t ));

	printk("cci_hw_id: 0x%x\r\n", smem_cci_smem_value->cci_hw_id);

	CCI_hardware_id = smem_cci_smem_value->cci_hw_id;	
}

int driver_get_hw_id(void)
{
	int hw_id;

	hw_id = CCI_hardware_id;
...
#ifdef CONFIG_CCI_PJID
static int CCI_project_id;

static void hw_project_version(void)
{
	cci_smem_value_t *smem_cci_smem_value;

	smem_cci_smem_value = smem_alloc( SMEM_CCI_SMEM_VALUE, sizeof( cci_smem_value_t ));

	printk("cci_project_id: 0x%x\r\n", smem_cci_smem_value->cci_project_id);

	CCI_project_id = smem_cci_smem_value->cci_project_id;
}
...
static struct snd_endpoint snd_endpoints_list[] = {
	...
	SND(NO_MIC_HEADSET, 28),
	SND(HEADSET_AND_SPEAKER, 29), // HEADSET_AND_SPEAKER = HS_SPK = 29
	SND(HAC, 30),
	SND(STEREO_HEADSET_NO_MIC_PLAYBACK, 31),
	SND(PLAYBACK_RING_TONE, 32),
	SND(FM_SPEAKER, 33),
	SND(FM_HEADSET, 34),
	SND(CURRENT, 36),
	// CCI Aaron Chen 2010.07.19 ----------------------------
};
#undef SND
...
static unsigned bt_config_power_on[] = {
	GPIO_CFG(107, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),	/* WAKE */
...
static unsigned bt_config_power_off[] = {
	GPIO_CFG(107, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),	/* WAKE */
...
static uint32_t camera_off_gpio_table[] = {
	...
	GPIO_CFG(1,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT1 */
...
static uint32_t camera_on_gpio_table[] = {
	...
	GPIO_CFG(1,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), /* DAT1 */
...
static struct msm_gpio sdc1_cfg_data[] = {
	{GPIO_CFG(51, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc1_dat_3"},
	{GPIO_CFG(52, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc1_dat_2"},
	{GPIO_CFG(53, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc1_dat_1"},
	{GPIO_CFG(54, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc1_dat_0"},
...
{
	{GPIO_CFG(51, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),"sdc1_dat_3"},
	{GPIO_CFG(52, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),"sdc1_dat_2"},
	{GPIO_CFG(53, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),"sdc1_dat_1"},
	{GPIO_CFG(54, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),"sdc1_dat_0"},
	{GPIO_CFG(55, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),"sdc1_cmd"},
	{GPIO_CFG(56, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),"sdc1_clk"},
};
...
/*for HW detection of SD card*/
static unsigned int msm_sdcc_status(struct device *dev)
{
	unsigned int status;
	status = (unsigned int) gpio_get_value(19);
	return !status; 
}
...
static struct mmc_platform_data msm7x2x_sdc1_data = {
	...
	.status		= msm_sdcc_status,
	.status_irq		= MSM_GPIO_TO_INT(19),
...
static struct mmc_platform_data msm7x2x_sdc2_data = {
	...
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.status			= k_wifi_status,
	.register_status_notify	= k_wifi_status_register,
...
#ifdef CONFIG_CCI_UUID
static int msm_modem_uuid_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int rc = -1;
	int len = 0;
	//char *buf = "0";
	...
	unsigned dwIMEIHigh = 0, dwIMEILow = 0;
	unsigned char uuid[20];
	unsigned char buf[20];

	rc = msm_proc_comm(PCOM_GET_IMEI, &dwIMEIHigh, &dwIMEILow);
	if( rc != 0 ) {
		printk(KERN_ERR "Reading IMEI not supported, use hard code IMEI \n");
		dwIMEIHigh = 0x15004400;
        	dwIMEILow = 0xFF002020;
        	rc = 1;
	}

	// Set the manufacturer ID for ACER
	uuid[0] = 0x00;
	uuid[1] = 0x90;
	uuid[2] = 0x47;
	uuid[3] = 0x00;
	...
	// Set the version/variant of the UUID
	uuid[6] = 1;
	uuid[7] = 8;

	// Set the 64-bits unique ID
	memcpy(uuid+8, &dwIMEIHigh, sizeof(dwIMEIHigh));
	memcpy(uuid+12, &dwIMEILow, sizeof(dwIMEILow));

	for(i=0;i<16;i++)
	{
		len += sprintf(buf + len, "%02X", uuid[i]);
	}
...
#ifdef CONFIG_CCI_HWID
static int msm_modem_hwid_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
	cci_hw_id_type hw_id;

	hw_id = driver_get_hw_id();
	switch(hw_id){
	case HW_ID_EMU:
		len += sprintf(page, "EMU\n");
		break;
	case HW_ID_EVT0:
		len += sprintf(page, "EVT0\n");
		break;
	case HW_ID_EVT1:
		len += sprintf(page, "EVT1\n");
		break;
	case HW_ID_EVT2:
		len += sprintf(page, "EVT2\n");
		break;
	case HW_ID_DVT1:
		len += sprintf(page, "DVT1\n");
		break;
	case HW_ID_DVT1_2:
		len += sprintf(page, "DVT1_2\n");
		break;
	case HW_ID_DVT2:
		len += sprintf(page, "DVT2\n");
		break;
	case HW_ID_DVT2_2:
		len += sprintf(page, "DVT2_2\n");
		break;
	case HW_ID_DVT3:
		len += sprintf(page, "DVT3\n");
		break;
	case HW_ID_PVT:
		len += sprintf(page, "PVT\n");
		break;
	case HW_ID_PVT1:
		len += sprintf(page, "PVT1\n");
		break;
	case HW_ID_2ND_ID_PVT:
		len += sprintf(page, "2ND_ID_PVT\n");
		break;
	default:
		len += sprintf(page, "Not a valid HW\n");
		printk(KERN_EMERG "Got an invalid HW version 0x%x\n", hw_id);
		break;
...
#ifdef CONFIG_CCI_PJID
static int msm_modem_pjid_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
	cci_project_id_type project_id;

	project_id = driver_get_pj_id();
	switch(project_id){
	case PROJECT_ID_K4:
		len += sprintf(page, "K4\n");
		break;
	case PROJECT_ID_K4H:
		len += sprintf(page, "K4H\n");
		break;
	case PROJECT_ID_K5:
		len += sprintf(page, "K5\n");
		break;
	case PROJECT_ID_CAP6:
		len += sprintf(page, "CAP6\n");
		break;
	case PROJECT_ID_CAP8:
		len += sprintf(page, "CAP8\n");
		break;
	case PROJECT_ID_CAP2:
		len += sprintf(page, "CAP2\n");
		break;
	case PROJECT_ID_CA23:
		len += sprintf(page, "CA23\n");
		break;
	case PROJECT_ID_CA26:
		len += sprintf(page, "CA26\n");
		break;

	default:
		len += sprintf(page, "Not a valid project\n");
		printk(KERN_EMERG "Got an invalid project version 0x%x\n", project_id);
		break;
...
#ifdef CONFIG_CCI_POWER_STATUS
static int msm_power_down_status_read_proc(
    char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	switch(smem_cci_smem_value->cci_power_down_event){
	    case CCI_POWER_ON_DOWN_FROM_ARM9_FATAL_ERROR:
			len += sprintf(page, "arm9_fatal\n");
			break;
		case CCI_POWER_ON_DOWN_FROM_POWER_KEY:
			len += sprintf(page, "power_key\n");
			break;
		case CCI_POWER_ON_DOWN_FROM_POWER_CUT:
			len += sprintf(page, "power_cut\n");
			break;
		case CCI_POWER_ON_DOWN_FROM_SW_RESET:
		case CCI_POWER_ON_DOWN_FROM_HW_RESET:
			len += sprintf(page, "sw-hw_reset\n");
		    break;
		default:
...
#ifdef CONFIG_CCI_MODEM_VERSION
static int msm_modem_version_read_proc(
    char *page, char **start, off_t off, int count, int *eof, void *data)
{
    cci_smem_value_t *smem_cci_smem_value = 0;
    char modem_version[20], modem_version_temp[20];

    smem_cci_smem_value = smem_alloc( SMEM_CCI_SMEM_VALUE, sizeof( cci_smem_value_t ) );
    //For ACER requirement
    if((CCI_project_id == PROJECT_ID_K4)||(CCI_project_id == PROJECT_ID_K4H)||(CCI_project_id == PROJECT_ID_K5))
    {
	strcpy(modem_version_temp, smem_cci_smem_value->modem_version);
	if(CCI_project_id == PROJECT_ID_K4)
	{
		strcpy(modem_version, "K4-");
	}
	else if(CCI_project_id == PROJECT_ID_K4H)
	{
		strcpy(modem_version, "K4H-");
	}
	else
	{
		strcpy(modem_version, "K5-");
	}
	strncat(modem_version, modem_version_temp,5);
	strcat(modem_version,"\n");
    }
    else
    {
	strcpy(modem_version, smem_cci_smem_value->modem_version);
	strcat(modem_version,"\n");
    }
    strcpy(page, modem_version);
    return strlen(page);
}
#endif
...
#endif
#ifdef CONFIG_CCI_HWID
	struct proc_dir_entry *hwid_entry;
#endif
#ifdef CONFIG_CCI_MODEM_VERSION
	struct proc_dir_entry *modem_entry;
#endif
#ifdef CONFIG_CCI_POWER_STATUS
	struct proc_dir_entry *power_entry;
#endif
#ifdef CONFIG_CCI_PJID
	struct proc_dir_entry *pjid_entry;
#endif
	if (socinfo_init() < 0)
		BUG();
#ifdef CONFIG_ARCH_MSM7X25
...
#ifdef CONFIG_CCI_UUID
	uuid_entry = create_proc_entry("uuid", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
	if (uuid_entry) {
		uuid_entry->read_proc = msm_modem_uuid_read_proc;
		uuid_entry->write_proc = NULL;
		uuid_entry->data = NULL;
	}
#endif

#ifdef CONFIG_CCI_HWID
	hwid_entry = create_proc_entry("hwid", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
	if (hwid_entry) {
		hwid_entry->read_proc = msm_modem_hwid_read_proc;
		hwid_entry->write_proc = NULL;
		hwid_entry->data = NULL;
	}
#endif

#ifdef CONFIG_CCI_MODEM_VERSION
modem_entry = create_proc_entry("modem_version", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
      if (modem_entry) {
          modem_entry->read_proc = msm_modem_version_read_proc;
          modem_entry->write_proc = NULL;
          modem_entry->data = NULL;
      }
#endif

#ifdef CONFIG_CCI_POWER_STATUS
	  power_entry = create_proc_entry("power_down_status",
                S_IRUGO | S_IWUSR | S_IWGRP, NULL);

      if (power_entry) {
          power_entry->read_proc = msm_power_down_status_read_proc;
          power_entry->write_proc = NULL;
          power_entry->data = NULL;
      }
#endif

#ifdef CONFIG_CCI_PJID
	pjid_entry = create_proc_entry("project_id", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
	if (pjid_entry) {
		pjid_entry->read_proc = msm_modem_pjid_read_proc;
		pjid_entry->write_proc = NULL;
		pjid_entry->data = NULL;
	}
#endif
