...
static void usb_chg_detect(struct work_struct *w)
{
	 ...
	 * */
	if (temp == USB_CHG_TYPE__WALLCHARGER) {
		dev_info(&ui->pdev->dev, "[USB][POWER] USB_CHG_TYPE__WALLCHARGER\n\n");
		pm_runtime_put_sync(&ui->pdev->dev);
		cci_android_charger_usb_change(1, 1);
		wake_unlock(&ui->wlock);
	}
	else
	{
		dev_info(&ui->pdev->dev, "[USB][POWER] USB_CHG_TYPE__SDP\n\n");
		cci_android_charger_usb_change(1, 2);		}
}
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				...
				cci_android_charger_usb_change(0, 0);
