...
char serial_number[MAX_STR_LEN] = "1234567890ABCDEF";
/* String Table */
...
static int get_product_id(struct android_dev *dev)
{
	...
	if (p) {
		for (i = 0; i < count; i++, p++) {
			...
			{
				printk("[USB]%s, PID:0x%x\n", __func__, p->product_id);
				return p->product_id;
			}
		}
	}
	/* use default product ID */
	printk("[USB]%s, use default PID:0x%x\n", __func__, dev->product_id);
...
static void android_config_functions(struct usb_function *f, int enable)
{
	...
	if (enable) {
		...
		{
			if(dev->adb_enabled)
				android_set_default_product(C4_PID_ADB_ENABLE);
			else
				android_set_default_product(C4_PID_ADB_DISABLE);
		}
}
#endif
...
void android_enable_function(struct usb_function *f, int enable)
{
		...
		printk("[USB]%s, f_name:%s, enable:%d\n", __func__, f->name, enable);
		printk("[USB]%s, pid:0x%x, adb_enable:%d\n", __func__, dev->product_id, dev->adb_enabled);
		...
		if (!!f->disabled != disable) {
			...
			if(!strcmp(f->name, "adb"))
			{
				...
				if(C4_PID_RNDIS_MODE == dev->product_id)
					return;
			}
...
static int android_debugfs_init(struct android_dev *dev)
{
	...
	android_debug_serialno = debugfs_create_file("serial_number", 0222,
						...
						&android_fops);
	printk(KERN_ERR "[USB]android_debug_root.d_flags=%d\n", android_debug_root->d_flags);
	printk(KERN_ERR "[USB]android_debug_serialno.d_mounted=%d\n", android_debug_serialno->d_mounted);
...
#ifdef CONFIG_DEBUG_FS
	printk(KERN_ERR "[USB]#ifdef CONFIG_DEBUG_FS\n");
	result = android_debugfs_init(dev);
	if (result)
		pr_debug("%s: android_debugfs_init failed\n", __func__);
#endif
	printk(KERN_ERR "[USB]dev->num_products=%d, dev->functions=%s, \
				dev->num_functions=%d, dev->version=%d", \
				dev->num_products, *dev->functions, dev->num_functions, dev->version);
