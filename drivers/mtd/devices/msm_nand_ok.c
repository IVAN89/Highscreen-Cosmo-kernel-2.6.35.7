...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (dev_found) {
		...
		pr_err("Found a supported NAND device\n");
		pr_err("NAND Id  : 0x%x\n", supported_flash[index].
			flash_id);
		pr_err("Buswidth : %d Bits\n", (wide_bus) ? 16 : 8);
		pr_err("Density  : %lld MByte\n", (mtd->size>>20));
		pr_err("Pagesize : %d Bytes\n", mtd->writesize);
		pr_err("Erasesize: %d Bytes\n", mtd->erasesize);
		pr_err("Oobsize  : %d Bytes\n", mtd->oobsize);
	} else {
	...
	pr_err("CFG0 Init  : 0x%08x \n", chip->CFG0);
	pr_err("CFG1 Init  : 0x%08x \n", chip->CFG1);
	pr_err("ECCBUFCFG  : 0x%08x \n", chip->ecc_buf_cfg);
