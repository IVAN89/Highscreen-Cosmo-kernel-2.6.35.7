...
//static ssize_t msm_fb_read(struct fb_info *info, char __user *buf, size_t count, loff_t *ppos); //change IVAN-ICE
//static ssize_t msm_fb_write(struct fb_info *info, const char __user *buf, size_t count, loff_t *ppos); //change IVAN-ICE
...
#endif /* NEVER */
    bl_lvl = mfd->panel_info.bl_min+(2*value*(mfd->panel_info.bl_max-mfd->panel_info.bl_min)+ MAX_BACKLIGHT_BRIGHTNESS)/(2*MAX_BACKLIGHT_BRIGHTNESS);

    printk("mfd->panel_info.bl_min=%d,mfd->panel_info.bl_max=%d,bl_lvl=%d,value=%d\n",mfd->panel_info.bl_min,mfd->panel_info.bl_max,bl_lvl,value);
	if (!bl_lvl && value)
		bl_lvl = 1;
...
static int msmfb_overlay_3d(struct fb_info *info, unsigned long *argp) //change IVAN-ICE
{
	int	ret;
